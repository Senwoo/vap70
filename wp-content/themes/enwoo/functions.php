    ...
    function enwoo_setup() {
        ...
        add_theme_support('recommend-plugins', array(
            'envothemes-demo-import' => array(
                ...
                'description' => esc_html__('Save time by importing our demo data: your website will be set up and ready to be customized in minutes.', 'enwoo'),
            ),
            'woocommerce' => array(
                'name' => 'WooCommerce',
                'active_filename' => 'woocommerce/woocommerce.php',
                /* translators: %s plugin name string */
                'description' => sprintf(esc_attr__('To enable shop features, please install and activate the %s plugin.', 'enwoo'), '<strong>WooCommerce</strong>'),
            ),
            'elementor' => array(
                'name' => 'Elementor',
                'active_filename' => 'elementor/elementor.php',
                /* translators: %s plugin name string */
                'description' => sprintf(esc_attr__('The most advanced frontend drag & drop page builder.', 'enwoo'), '<strong>Elementor</strong>'),
            ),
        ));
    }
...
function enwoo_fonts_url() {
     ...
     */
    $font = _x('on', 'Lato: on or off', 'enwoo');
...
if (!function_exists('enwoo_title_logo')) {
    ...
    add_action('enwoo_header', 'enwoo_title_logo', 10);
    ...
    function enwoo_title_logo() {
        ...
        <div class="site-heading <?php echo esc_attr(class_exists('WooCommerce') == true ? 'col-md-3' : 'col-md-4' ); ?>" >    
            ...
            <?php if (is_active_sidebar('enwoo-header-area') && !class_exists('WooCommerce')) { // Enabled only when WooCommerce is not active. ?>
...
if (!function_exists('enwoo_menu')) {
    ...
    function enwoo_menu() {
        ?>
        <div class="menu-heading row <?php echo esc_attr(class_exists('WooCommerce') == true ? 'col-md-6' : 'col-md-12' ); ?>">
...
add_action('enwoo_header', 'enwoo_head_start', 25);

function enwoo_head_start() {
    echo '<div class="header-right col-md-3" >';
}

add_action('enwoo_header', 'enwoo_head_end', 80);
...
if (!function_exists('enwoo_menu_button')) {
    ...
    add_action('enwoo_header', 'enwoo_menu_button', 28);
...
require_once( trailingslashit(get_template_directory()) . 'lib/extras/extras.php' );
require 'lib/extras/plugin-update-checker/plugin-update-checker.php';
$myUpdateChecker = Puc_v4_Factory::buildUpdateChecker(
	'http://enwoo-wp.com/wp-content/uploads/theme-update/theme.json',
